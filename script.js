...
// --- ГЛОБАЛЬНЫЕ ЭЛЕМЕНЕНТЫ (будут определены после инициализации) ---
...
function handleLogin() {
    ...
    if (loginElements.input.value === SECRET_KEY) {
        sessionStorage.setItem('is-authenticated', 'true'); // Save session
...
function initializeApp() {
    ...
    function showLyrics(promptText) { lyricsModal.content.textContent = promptText; lyricsModal.overlay.style.display = 'flex'; }
    ...
    async function deleteSong(songId, cardElement) { try { const response = await fetch(`/api/songs/${songId}`, { method: 'DELETE' }); if (!response.ok) throw new Error('Failed to delete'); cardElement.style.transition = 'opacity 0.3s, transform 0.3s'; cardElement.style.opacity = '0'; cardElement.style.transform = 'translateX(-20px)'; setTimeout(() => { cardElement.remove(); playlist = playlist.filter(p => p.songData.id !== songId); }, 300); } catch (e) { console.error("Could not delete song", e); } }
    
    async function toggleFavorite(songId, cardElement) {
        const songIndex = playlist.findIndex(p => p.songData.id === songId);
        if (songIndex === -1) return;
        const songInfo = playlist[songIndex];
        const newStatus = !songInfo.songData.is_favorite;
        try {
            const response = await fetch(`/api/songs/${songId}/favorite`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ is_favorite: newStatus }) });
            ...
            if (favButton) { favButton.innerHTML = `<i class="fas ${newStatus ? 'fa-heart' : 'far fa-heart'}"></i> ${newStatus ? 'Убрать из избранного' : 'В избранное'}`; }
            cardElement.querySelector('.song-menu').classList.remove('active');
    ...
    function addSongToList(songInfo) {
        ...
        menuItems.forEach(item => { const li = document.createElement('li'); li.className = 'menu-item ' + (item.className || ''); li.innerHTML = `<i class="fas ${item.icon}"></i> ${item.text}`; li.onclick = item.action; menu.appendChild(li); });
        songListContainer.appendChild(card);
        setTimeout(() => { const styleContent = card.querySelector('.song-style-content'); if (styleContent && styleContent.scrollHeight > styleContent.clientHeight) { styleContent.classList.add('collapsed'); const toggleBtn = document.createElement('button'); toggleBtn.className = 'toggle-description-btn'; toggleBtn.textContent = '[ развернуть ]'; toggleBtn.onclick = () => { const isCollapsed = styleContent.classList.toggle('collapsed'); toggleBtn.textContent = isCollapsed ? '[ развернуть ]' : '[ свернуть ]'; }; styleContent.parentElement.appendChild(toggleBtn); } }, 10);
    }
    ...
    async function startPolling(taskId) {
        ...
        pollingInterval = setInterval(async () => {
            ...
            try {
                ...
                if (successStates.includes(statusLowerCase)) { clearInterval(pollingInterval); if(progressBar) progressBar.style.width = `100%`; updateStatus("✅ Задача выполнена!", true); const placeholder = document.getElementById(`placeholder-${taskId}`); if(placeholder) placeholder.remove(); await loadSongsFromServer(); await handleApiCall("/api/chat/credit", { method: "GET" }, true); } else if (pendingStates.includes(statusLowerCase)) { updateStatus(`⏳ Статус: ${taskData.status}. Следующая проверка через 10 сек...`); } else { handleTaskError(taskId, taskData.errorMessage || `API вернул статус сбоя: ${taskData.status}`); }
            } catch (error) { handleTaskError(taskId, "Сетевая ошибка или ошибка парсинга ответа."); }
        }, 10000);
    }
    
    async function loadSongsFromServer() { try { const response = await fetch('/api/songs'); const songs = await response.json(); songListContainer.innerHTML = ''; playlist = songs; if (songs.length > 0) { emptyListMessage.style.display = 'none'; songs.forEach(songInfo => addSongToList(songInfo)); } else { songListContainer.innerHTML = '<p id="empty-list-message">Здесь будут отображаться ваши сгенерированные песни.</p>'; } } catch(e) { console.error("Не удалось загрузить песни с сервера", e); } }
    ...
    function setupEventListeners() {
        ...
        document.getElementById("generate-music-form").addEventListener("submit", function(e) {
            ...
            if (payload.customMode) { payload.title = titleInput.value; payload.style = styleTextarea.value; if (!payload.instrumental) payload.prompt = promptTextarea.value; const negativeTags = document.getElementById("g-negativeTags").value; if(negativeTags) payload.negativeTags = negativeTags; const vocalGender = document.getElementById("g-vocalGender").value; if (vocalGender) payload.vocalGender = vocalGender; } else { if (!payload.instrumental) payload.prompt = descriptionInput.value; }
            handleApiCall("/api/generate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }, false, true);
        });
    }

    toggleGeneratorMode(); updateCharacterLimits(modelValueInput.value); handleApiCall("/api/chat/credit", { method: "GET" }, true); loadSongsFromServer(); setupEventListeners();
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    accessKeyInput.addEventListener('input', () => { if (hideKeyTimeout) clearTimeout(hideKeyTimeout); accessKeyInput.type = 'text'; hideKeyTimeout = setTimeout(() => { accessKeyInput.type = 'password'; }, 500); });
    if (sessionStorage.getItem('is-authenticated') === 'true') { document.getElementById('login-overlay').style.display = 'none'; const appTemplate = document.getElementById('app-template'); const appContainer = document.getElementById('app-container'); appContainer.appendChild(appTemplate.content.cloneNode(true)); appContainer.style.display = 'block'; initializeApp(); }
});

function createPlaceholderCard(taskId) { const emptyListMessage = document.getElementById('empty-list-message'); const songListContainer = document.getElementById('song-list-container'); if (emptyListMessage) emptyListMessage.style.display = 'none'; const card = document.createElement('div'); card.className = 'song-card placeholder'; card.id = `placeholder-${taskId}`; card.innerHTML = `<div class="song-cover"><div class="song-duration">--:--</div></div><div class="song-info"><span class="song-title">Генерация...</span><span class="song-style">Пожалуйста, подождите</span><div class="progress-bar-container"><div class="progress-bar-inner" id="progress-${taskId}"></div></div></div>`; songListContainer.prepend(card); }
function updateCharCounter(e,t){const o=document.getElementById(t);o&&(o.textContent=`${e.value.length} / ${e.maxLength}`)}
function openTab(event, tabName) { if (currentTabName === tabName) return; const currentActiveTab = document.getElementById(currentTabName); const newTab = document.getElementById(tabName); document.querySelectorAll(".tab-button").forEach(btn => btn.classList.remove("active")); event.currentTarget.classList.add("active"); if (currentActiveTab) { currentActiveTab.classList.add('exiting'); currentActiveTab.addEventListener('animationend', () => { currentActiveTab.classList.remove('active', 'exiting'); currentActiveTab.style.display = 'none'; newTab.style.display = 'flex'; newTab.classList.add('active'); currentTabName = tabName; }, { once: true }); } if (tabName === 'logs') document.getElementById('logs-notification-dot').style.display = 'none'; }
async function handleApiCall(endpoint, options, isCreditCheck = false, isGeneration = false) { const responseOutput = document.getElementById("response-output"); const statusContainer = document.getElementById("status-container"); if(!isCreditCheck) { statusContainer.innerHTML = '<p class="status-message">Ожидание запуска задачи...</p>'; responseOutput.textContent = "Выполняется запрос..."; } if (pollingInterval && !isCreditCheck) clearInterval(pollingInterval); try { const response = await fetch(endpoint, options); const result = await response.json(); if (response.ok) { if(!isCreditCheck) responseOutput.textContent = JSON.stringify(result, null, 2); if (isCreditCheck && result.data !== undefined) { document.getElementById("credits-value").textContent = result.data; document.getElementById("credits-container").style.display = "inline-flex"; } if (isGeneration && result.data && result.data.taskId) { startPolling(result.data.taskId); } else if (isGeneration) { updateStatus(`🚫 Ошибка запуска: ${result.message || 'Не удалось получить taskId.'}`, false, true); } } else { if(!isCreditCheck) responseOutput.textContent = `🚫 Ошибка ${response.status}:\n\n${JSON.stringify(result, null, 2)}`; updateStatus(`🚫 Ошибка запуска: ${result.message || 'Сервер вернул ошибку.'}`, false, true); } } catch (error) { if(!isCreditCheck) responseOutput.textContent = "💥 Сетевая ошибка или ошибка парсинга JSON:\n\n" + error.message; updateStatus(`💥 Критическая ошибка: ${error.message}`, false, true); } }
function markInputAsError(element) { element.classList.add('input-error'); setTimeout(() => { element.classList.remove('input-error'); }, 1500); }
function validateGenerateForm() { const titleInput = document.getElementById('g-title'), styleInput = document.getElementById('g-style'), promptInput = document.getElementById('g-prompt'), descriptionInput = document.getElementById('g-song-description'); [titleInput, styleInput, promptInput, descriptionInput].forEach(el => el.classList.remove('input-error')); const isCustom = document.getElementById("g-customMode").checked, isInstrumental = document.getElementById("g-instrumental").checked; let allValid = true; if (isCustom) { if (titleInput.value.trim() === '') { markInputAsError(titleInput); allValid = false; } if (styleInput.value.trim() === '') { markInputAsError(styleInput); allValid = false; } if (!isInstrumental && promptInput.value.trim() === '') { markInputAsError(promptInput); allValid = false; } } else { if (!isInstrumental && descriptionInput.value.trim() === '') { markInputAsError(descriptionInput); allValid = false; } } return allValid; }
...
function updateStatus(message, isSuccess = false, isError = false) { const statusContainer = document.getElementById("status-container"); if(statusContainer) statusContainer.innerHTML = `<div class="status-message ${isSuccess ? 'success' : ''} ${isError ? 'error' : ''}">${message}</div>`; }