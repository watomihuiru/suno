...
function addSongToList(songInfo) {
    ...
    menuItems.forEach(item => { const li = document.createElement('li'); li.className = 'menu-item ' + (item.className || ''); li.innerHTML = `<i class="${item.icon.startsWith('fa-') ? 'fas' : ''} ${item.icon}"></i> ${item.text}`; li.onclick = item.action; menu.appendChild(li); });
...
function setupEventListeners() {
    // Main tabs
    document.querySelectorAll('.main-card .tabs .tab-button').forEach(btn => btn.addEventListener('click', (e) => openTab(e, e.target.dataset.tab)));
    ...
    document.getElementById("extend-music-form").addEventListener("submit", (e) => {
        e.preventDefault();
        const payload = {
            audioId: document.getElementById("e-audioId").value,
            continueAt: document.getElementById("e-continueAt").value,
    ...
    document.getElementById("upload-cover-form").addEventListener("submit", (e) => {
        ...
        const payload = {
            ...
            prompt: document.getElementById("uc-prompt").value
        };
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (sessionStorage.getItem('is-authenticated') === 'true') {
        ...
        const appTemplate = document.getElementById('app-template');
        document.getElementById('app-container').appendChild(appTemplate.content.cloneNode(true)).style.display = 'block';
        initializeApp();
    }
});